...
A postura de segurança da fase 3 herda a bateria da fase 2 e o princípio de **evidência local primeiro** enquanto a cota do GitHub Actions estiver esgotada: os scanners executáveis localmente foram rodados na HEAD atual e registrados abaixo com data. Os scanners que dependem do CI (trivy, gitleaks, CodeQL, OWASP ZAP) estão commitados nos workflows ([`security.yml`](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/.github/workflows/security.yml), [`ci.yml`](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/.github/workflows/ci.yml), [`full-test-ci.yml`](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/.github/workflows/full-test-ci.yml) e CodeQL default setup) e voltam a rodar a cada push quando a cota renovar (pendência 3, seção 9).
...
| trivy · gitleaks · CodeQL · OWASP ZAP | SCA imagem / segredos / SAST semântico / DAST | imagem Docker, árvore git, código, API viva | **Commitados no CI, aguardando cota do Actions** — última execução verde registrada em [`scan-fase-2.md`](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/docs/seguranca/scan-fase-2.md) (herança do snapshot); paridade local do ZAP disponível via `make dast` |
