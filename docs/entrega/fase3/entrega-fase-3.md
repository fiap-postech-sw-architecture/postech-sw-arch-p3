...
A postura de segurança da fase 3 herda a bateria da fase 2 e o princípio de **evidência local primeiro** enquanto a cota do GitHub Actions estiver esgotada: os scanners executáveis localmente foram rodados na HEAD atual e registrados abaixo com data. Os scanners que restam dependentes do CI (trivy, gitleaks) estão commitados nos workflows ([`security.yml`](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/.github/workflows/security.yml), [`ci.yml`](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/.github/workflows/ci.yml), [`full-test-ci.yml`](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/.github/workflows/full-test-ci.yml) — e o CodeQL default setup, que é configuração do próprio GitHub, não workflow versionado) e voltam a rodar a cada push quando a cota renovar (pendência 3, seção 9).
