...
Repositórios privados no GitHub (organização `fiap-postech-sw-architecture`), compartilhados com o usuário `soat-architecture` conforme exigido pelo enunciado — confirmado no repositório principal (herdado da fase 2) e **pendente de conferência nos quatro repositórios novos** (pendência 2, seção 9). São os quatro repositórios exigidos pelo enunciado, mais um quinto de processo:
...
Cada um dos quatro repositórios exigidos tem README com propósito, tecnologias, passos de execução e deploy, diagrama da arquitetura específica e pipelines `ci.yml` + `cd.yml` com deploy automático por branch. Em app e lambda, push em `homolog` vai para o ambiente de homologação e push em `main` vai para produção. Nos repos de infra (`infra-k8s`/`infra-db`), push em `homolog` roda só `terraform plan` (estágio de homologação de infra); o apply automático fica na `main`, porque um único Learner Lab não comporta infra duplicada (Adendo (b) do [ADR-033](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/docs/arquitetura/adr/fase3/033-cicd-multi-repo.md)). A proteção técnica da branch `main` mostrou-se inviável na organização atual (plano free + repositórios privados — HTTP 403 "Upgrade to GitHub Pro"). Por isso o fluxo de PR obrigatório vale por convenção documentada, com o gate local como freio — decisão e mitigação no [Adendo do ADR-033](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/docs/arquitetura/adr/fase3/033-cicd-multi-repo.md#adendo-2026-07-11--limitações-constatadas-e-decisões-complementares).

### Estado do CI/CD e gate local espelho

A cota de GitHub Actions da organização está esgotada: os pipelines dos quatro repositórios estão **commitados e corretos, porém não executáveis até a cota renovar** (plano de desbloqueio na seção 9). Enquanto isso, o gate local espelho é obrigatório antes de cada push ([RFC-003 §6](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/docs/arquitetura/rfc/fase3/rfc-003-gateway-serverless-observabilidade.md)) — resultado na HEAD atual (2026-07-11):

| Repo | Gate local | Resultado |
|---|---|---|
| `p3` (app) | `make check` (lint, import-linter, mypy, bandit, testes) | Verde — **1.834 testes**, cobertura **96,39%** (gate ≥ 95%) |
...
A postura de segurança da fase 3 herda a bateria da fase 2 e o princípio de **evidência local primeiro** enquanto a cota do GitHub Actions estiver esgotada: os scanners executáveis localmente foram rodados na HEAD atual e registrados abaixo com data. Os scanners que restam dependentes do CI (trivy, gitleaks) estão commitados nos workflows ([`security.yml`](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/.github/workflows/security.yml), [`ci.yml`](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/.github/workflows/ci.yml), [`full-test-ci.yml`](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/.github/workflows/full-test-ci.yml) e CodeQL default setup) e voltam a rodar a cada push quando a cota renovar (pendência 3, seção 9).
...
| pip-audit (`uv run --with pip-audit pip-audit`, p3) | SCA (Software Composition Analysis, vulnerabilidades em dependências) | ambiente resolvido do `uv.lock` | **0 vulnerabilidades conhecidas** (apenas o pacote local `pytstop 0.2.0` não auditável — não publicado no PyPI, esperado) |
| OWASP ZAP (baseline) | DAST | API viva (stack compose dedicada) | Executado localmente em 11/07/2026 via `make dast`: FAIL 0 · WARN 0 · PASS 65 em 58 URLs (2 regras IGNORE do baseline) |
...
| SonarQube (community, self-hosted) | Análise estática/qualidade — scan manual de fechamento (não é gate de CI por decisão, [TD-010/ADR-011](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/docs/arquitetura/adr/011-pipeline-seguranca-analise-estatica.md)) | `src/` (7.518 LoC) + coverage | Executado localmente em 11/07/2026: Quality Gate Passed — 0 bugs, 0 vulnerabilities, ratings A/A/A, 0% duplicação; 3 security hotspots revisados como seguros (mesmos da fase 2, justificativas inline no código); cobertura 94,6% no denominador do Sonar (o gate real mede 96,4% — divergência de universo documentada no `sonar-project.properties`) |
| trivy · gitleaks | SCA de imagem / segredos | imagem Docker, árvore git | **Commitados no CI, aguardando cota do Actions** — última execução verde registrada em [`scan-fase-2.md`](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/docs/seguranca/scan-fase-2.md) (herança do snapshot) |
...
| ID | Requisito | Implementação / decisão | Evidência de verificação |
|---|---|---|---|
| RNF-025 | 4 repositórios com CI/CD e deploy automático (homolog/produção); main protegida; PRs obrigatórios | Repos `p3`, `p3-lambda`, `p3-infra-k8s`, `p3-infra-db`, cada um com `ci.yml` + `cd.yml`; app/lambda: `homolog` → homologação e `main` → produção; infra: `homolog` = `terraform plan`, apply só na `main` (Adendo (b) do ADR-033). **Branch protection técnica indisponível** na organização atual (plano free + repositórios privados, HTTP 403) — PRs obrigatórios valem por convenção documentada, mesma condição da fase 2 (Adendo (a)) ([ADR-033](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/docs/arquitetura/adr/fase3/033-cicd-multi-repo.md)) | Workflows commitados nos 4 repos; **cota do Actions esgotada — gate local espelho verde nos 4 repos** (tabela da seção 2); **branch protection inviável na org free** — PR obrigatório por convenção documentada, mitigação no [Adendo do ADR-033](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/docs/arquitetura/adr/fase3/033-cicd-multi-repo.md#adendo-2026-07-11--limitações-constatadas-e-decisões-complementares) |
...
| RNF-029 | Logs estruturados JSON com correlação entre requisições | structlog JSON com scrub de PII (herdado); middleware passa a **aceitar o `X-Request-ID` externo** vindo do gateway (`src/compartilhado/interfaces/middleware.py`), gerando UUID só na ausência; agregação com Loki + Promtail e consulta por `request_id` no Grafana ([RFC-003 §7](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/docs/arquitetura/rfc/fase3/rfc-003-gateway-serverless-observabilidade.md)) | Testes do middleware no `make check` (1.834 testes); consulta LogQL documentada no [`k8s/README.md`](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/k8s/README.md); demonstração no bloco 6 do vídeo |
...
| 2 | Confirmar `soat-architecture` como colaborador nos **4 repositórios novos** (`p3-lambda`, `p3-infra-k8s`, `p3-infra-db`, `p3-docs`) — no `p3` a colaboração é herdada da fase 2, conferir também | GitHub → Settings → Collaborators de cada repo |
| 3 | Cota do GitHub Actions: quando renovar, disparar os pipelines dos 4 repos e registrar as runs verdes (o gate local espelho já está verde — seção 2) | [Plano de desbloqueio 2](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3-docs/blob/main/docs/superpowers/plans/2026-07-11-desbloqueio-2-cota-actions.md) |
| 4 _(necessária para o bloco "deploy automatizado" do vídeo)_ | Deploy AWS de ponta a ponta (RDS → EKS → app → gateway/lambda) numa sessão do Academy — OU, no mínimo, um run verde do `cd.yml` disparado por push (Desbloqueio 2, quando a cota renovar), que também é deploy automatizado real. O `make cd-local` sozinho é comando manual e só serve de fallback narrado | [Plano de desbloqueio 1](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3-docs/blob/main/docs/superpowers/plans/2026-07-11-desbloqueio-1-aws-deploy.md) + runbook [`aws-academy-setup.md`](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3-docs/blob/main/docs/runbooks/aws-academy-setup.md) |
