...
| ID | Requisito | Implementação / decisão | Evidência de verificação |
|---|---|---|---|
| RNF-025 | 4 repositórios com CI/CD e deploy automático (homolog/produção); main protegida; PRs obrigatórios | Repos `p3`, `p3-lambda`, `p3-infra-k8s`, `p3-infra-db`, cada um com `ci.yml` + `cd.yml`; app/lambda: `homolog` → homologação e `main` → produção; infra: `homolog` = `terraform plan`, apply só na `main` (Adendo (b) do ADR-033). PR, checks e aprovação manual são obrigatórios por convenção; a proteção técnica depende de permissão `admin` (Adendo (g) do [ADR-033](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p3/blob/main/docs/arquitetura/adr/fase3/033-cicd-multi-repo.md)) | Deploy automático de produção comprovado nos quatro runs da seção 2; proteção técnica da `main` permanece como pendência humana explícita |
...
| 2 | ~~Adicionar `soat-architecture`~~: acesso de leitura confirmado nos cinco repositórios em 07/09/2026 | GitHub → Settings → Collaborators |
| 3 | Ativar a proteção técnica da `main`; a conta operacional possui `write`, mas a configuração exige `admin` | GitHub → Settings → Branches de cada repositório |
