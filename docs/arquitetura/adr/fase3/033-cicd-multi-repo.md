...
- **Espelho local obrigatório enquanto a cota do Actions estiver esgotada**: os pipelines ficam commitados e corretos, mas não executáveis; antes de cada push roda-se o gate local equivalente — `make check` no app e alvo `make gate` equivalente nos demais repos (fmt/validate/tflint, sam validate, testes). Quando a cota renovar, o CI passa a ser o gate canônico sem mudança nos gates.
