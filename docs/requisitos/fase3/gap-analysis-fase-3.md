...
| RN-021 | Token emitido pela function é o aceito pelas APIs protegidas | App valida tokens do próprio emissor (`jwt_service.py:60-75`, claims sub/email/papel/jti) | Dois emissores potenciais (lambda × app) | Dois emissores com públicos disjuntos (app=usuários internos; lambda=clientes), segredo e validador únicos — ADR-028 |
